...
@section('content')
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-header">{{ __('tables.product') }}</div>
                    <div class="card-body">
                        <a href="{{ url('/admin/product/create') }}" class="btn btn-success btn-sm" title="{{ __('crud.AddNew') }}">
                            ...
                            <table class="table">
                                ...
                                @foreach($product as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->name }}</td><td>{{ $item->category_id }}</td><td>{{ $item->supplier_id }}</td>
                                        <td>
                                            <a href="{{ url('/admin/product/' . $item->id) }}" title="{{ __('crud.view') }}"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> {{ __('crud.view') }}</button></a>
                                            <a href="{{ url('/admin/product/' . $item->id . '/edit') }}" title="{{ __('crud.edit') }}"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> {{ __('crud.edit') }}</button></a>
